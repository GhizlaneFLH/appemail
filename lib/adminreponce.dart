...
class Email {
  ...
  final String email;
  Email(this.sender, this.subject, this.body, this.date, this.email)
      : formattedDate =
            DateFormat('yyyy-MM-dd HH:mm').format(date); // Format the date
}

class EmailLReponce extends StatefulWidget {
  @override
  _EmailListScreenState createState() => _EmailListScreenState();
}

class _EmailListScreenState extends State<EmailLReponce> {
  List<Email> emailList = [];
Future<void> fetchEmails() async {
    var url = 'http://192.168.0.183/TEMACONCEPT/sendfetch.php';
